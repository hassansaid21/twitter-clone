...
export default function ImageEditor({
  ...
  const [draftSettings, setDraftSettings] = useState<Settings>(media.settings);
    ...
    <div className="fixed inset-0 bg-[#242D34]/50 z-10 flex items-center justify-center ">
      <div className="bg-black w-[90vw] h-[90vh] md:w-auto rounded-xl flex  flex-col gap-8 p-12 ">
        ...
        <div className="flex justify-between items-center ">
          <button
            onClick={onClose}
            className="px-2 py-1 rounded-full hover:bg-bgHover  "
          >
            ←
          </button>
            ...
            <div className="flex gap-6">
              {(["original", "wide", "square"] as const).map((type) => (
                <button
                  ...
                  className={
                    draftSettings.type === type ? "text-blue-400" : "text-white"
                  }
                >
                  {type}
                </button>
